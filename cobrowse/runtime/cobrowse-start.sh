...
wait_for() {
  local desc="$1" cmd="$2" timeout_ds="$3"  # timeout in deciseconds (0.1s units)
  ...
  echo "  FAIL: $desc (timeout after $(echo "scale=1; $timeout_ds/10" | bc)s)"
  return 1
}
...
if ! wait_for "Xvfb responding on display :${DISPLAY_NUM}" "xdpyinfo -display :${DISPLAY_NUM}" 30; then
...
if ! wait_for "VNC server listening on port ${VNC_PORT}" "ss -tlnp | grep -q :${VNC_PORT}" 30; then
...
if ! wait_for "noVNC listening on port ${NOVNC_PORT}" "ss -tlnp | grep -q :${NOVNC_PORT}" 30; then
...
if ! wait_for "Chromium CDP responding on port ${CDP_PORT}" "curl -s http://localhost:${CDP_PORT}/json/version" 100; then
